...
const regex = /(yarn add|npx|{.*?}|const|import|from|if|else|return|nativecn|=>|{|}|:|=|'|"|~|;|\[|]|\.|\?|\(|\)|@|\/|<|>|\/>)/g;
const formatText = (text: string) => {
  ...
  return parts.map((part, index) => {
    ...
    if (regex.test(part)) {
      return <P key={index} className="font-normal text-gray-400">{part}</P>;
    }

    return <P key={index} className="font-normal">{part}</P>;
  });
};
